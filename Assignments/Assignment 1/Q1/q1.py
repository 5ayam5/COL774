...
if __name__ == '__main__':
    ...
    for i, eta in zip(range(3), [0.001, 0.025, 0.1]):
        Theta, iterations, Thetas = gradient_descent(X, Y, eta, epsilon)
        axes_e[i].set_title('Contours')
        axes_e[i].set_xlabel('theta_0')
        axes_e[i].set_ylabel('theta_1')
        axes_e[i].contour(theta0, theta1, J, 100)
        learn, = axes_e[i].plot(*Thetas[:2, :iterations+1], color='red',
                                linestyle='None', marker='x',
                                label='learning theta')
        axes_e[i].legend()
    fig_e.tight_layout()
